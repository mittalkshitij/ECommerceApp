...
class CartAdapter(var context : Context, var cartItems: ArrayList<Items>)
    : RecyclerView.Adapter<CartAdapter.MyCartViewHolder>() {

    class MyCartViewHolder(var binding : ItemCartBinding) : RecyclerView.ViewHolder(binding.root) {

        fun bindingCartViews(cartItems: Items){

            binding.titleTextViewCart.text = cartItems.title
            binding.priceTextViewCart.text = "Price : $" + cartItems.price.toString()
